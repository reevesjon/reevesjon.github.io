...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:Pr0j3ctB33h1v3@localhost/4050_test' #replace with personal db credentials
...
@app.route('/manager2_data') #Swapped this to the data analytics page -Jon
def manager2_data():
...
#New route for manager2
@app.route('/manager2_dashboard/<int:user_id>')
def manager2_dashboard(user_id):
    user = User.query.get(user_id)
    tasks = Task.query.filter_by(assigned_to=user_id).all()
    ...
    return render_template('manager2_dashboard.html', user=user, tasks=tasks)

@app.route('/user_projectview/<int:user_id>')
def user_projectview(user_id):
    user = User.query.get(user_id)
    tasks = Task.query.filter_by(assigned_to=user_id).all()
    ...
    return render_template('user_projectview.html', user=user, tasks=tasks)
        ...
        if user:
            if user.password == password:  # Direct comparison, consider replacing with hashed passwords
                ...
                if 'sysadmin' in username.lower():
                    ...
                    return redirect(url_for('manager_landing')) #New route for manager dashboard (now managerlanding instead of manger1_dashboard)
                elif 'manager2' in username.lower():
                    return redirect(url_for('manager2_dashboard', user_id=user.user_id))
                else:
                    return redirect(url_for('user_landing', user_id=user.user_id))
...
@app.route('/userprojectview') 
def user_project_view():
    return render_template('userprojectview.html') 
